...
class TinkoffCreditCardParser(BaseParser):
    """
    Парсер кредитных карт Тинькофф Банка для частных лиц.

    Наследуется от BaseParser и реализует специфичную логику парсинга
    страницы с кредитными картами Тинькофф Банка.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Инициализация парсера Тинькофф Банка кредитных карт.
    ...
    async def parse_page(self, url: str) -> dict[str, Any]:
        """
        Парсинг страницы с кредитными картами Тинькофф Банка.
    ...
    async def _extract_cards(self, page: Page) -> list[dict[str, Any]]:
        ...
        """
        await self.random_delay(0.3, 0.5)
        # Ожидаем появления карточек на странице (с меньшим таймаутом)
        try:
        ...
        if cards_count == 0:
            ...
            title_elements = page.locator('h3[data-test="htmlTag title"]')
            ...
            if title_count > 0:
                ...
                card_elements = page.locator(
                    'div[data-test="atom-container"]:has(h3[data-test="htmlTag title"])'
                )
                cards_count = await card_elements.count()
        ...
        for i in range(cards_count):
            try:
                ...
                # Это позволяет пропустить пустые контейнеры без лишних операций
                title_locator = card.locator('h3[data-test="htmlTag title"]').first
                if await title_locator.count() == 0:
                    # Пропускаем контейнеры без названия
                    continue

                # Прокрутка к карточке для загрузки, если нужно (с коротким таймаутом)
                try:
                    await asyncio.wait_for(
                        card.scroll_into_view_if_needed(), timeout=2.0
                    )
                except (Exception, asyncio.TimeoutError):
                    # Если не удалось прокрутить за 2 секунды, продолжаем - элемент может быть уже виден
                    pass
