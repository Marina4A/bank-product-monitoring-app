...
class GazprombankDebitCardParser(BaseParser):
    """
    Парсер дебетовых карт Газпромбанка для частных лиц.

    Наследуется от BaseParser и реализует специфичную логику парсинга
    страницы с дебетовыми картами Газпромбанка.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Инициализация парсера Газпромбанка дебетовых карт.
    ...
    async def parse_page(self, url: str) -> dict[str, Any]:
        """
        Парсинг страницы с дебетовыми картами Газпромбанка.
        ...
        # Легкая прокрутка для загрузки lazy-loaded элементов
        try:
    ...
    async def _extract_cards(self, page: Page) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if await product_listing.count() > 0:
                ...
                product_cards = product_listing.locator(
                    ".product_listing_base_card-a65"
                )
        ...
        try:
            ...
            if await benefit_container.count() > 0:
                ...
                if await benefit_title_locator.count() > 0:
                    ...
                    label = await benefit_desc_locator.text_content() if await benefit_desc_locator.count() > 0 else None
            ...
            if await details_link_locator.count() > 0:
                ...
                if await details_link_locator_alt.count() > 0:
                    ...
                    if details_link:
                        if not details_link.startswith("http"):
                            details_link = (
                                f"https://www.gazprombank.ru{details_link}"
                            )
                        card_data["details_link"] = details_link
        except Exception:
